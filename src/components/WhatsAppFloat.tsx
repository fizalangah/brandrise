  ...
  return (
    <a
      href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(
        "Salaam BrandRise! Mujhe meri business ke liye website / automation chahiye."
      )}`}
      target="_blank"
      rel="noopener noreferrer"
      aria-label="Chat with BrandRise on WhatsApp"
      className="fixed bottom-5 right-5 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-emerald-500 text-white shadow-xl shadow-emerald-500/30 transition hover:scale-105 hover:bg-emerald-600"
    >
      <span
        aria-hidden="true"
        className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-40"
      />
      <svg className="relative" width="26" height="26" viewBox="0 0 24 24" fill="currentColor">
